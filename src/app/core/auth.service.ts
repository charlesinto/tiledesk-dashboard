...
export class AuthService {
  ...
  public project_bs: BehaviorSubject<Project> = new BehaviorSubject<Project>(
    null
  );
  public settingSidebarIsOpned: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(true);
  public nativeBotSidebarIsOpened: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(null);
  public tilebotSidebarIsOpened: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(null);
  public botsSidebarIsOpened: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(null);
  public isChromeVerGreaterThan100: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(null);
  ...
  constructor(
    ...
    private scriptService: ScriptService // public ssoService: SsoService
  ) {
    this.logger.log(
  ...
  public checkTrialExpired(): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      this.logger.log(
        "[AUTH-SERV] »> »> PROJECT-PROFILE GUARD (WF in AUTH SERV) called checkTrialExpired!"
      );
    ...
    if (storedUser !== null) {
      ...
      window.addEventListener("load", () => {
        try {
          ...
          this.logger.error(
            "[AUTH-SERV] Calling tiledesk_widget_login err",
            err
          );
        }
      });
  ...
  toggleSettingsSidebar(isopened) {
    this.logger.log(
      "[AUTH-SERV] - TOGGLE SETTINGS SIDEBAR IS OPENED ",
      isopened
    );
  ...
  toggleNativeBotSidebar(isopened) {
    this.logger.log(
      "[AUTH-SERV] - TOGGLE NATIVE BOT SIDEBAR IS OPENED ",
      isopened
    );
  ...
  checkStoredProjectAndPublishIfPublishedProjectIsNull() {
    this.project_bs.subscribe((prjct) => {
      this.logger.log(
        "[AUTH-SERV] - PROJECT FROM SUBSCRIPTION TO project_bs ",
        prjct
      );
      ...
      if (prjct == null) {
        this.logger.log(
          "[AUTH-SERV] - PROJECT FROM SUBSCRIPTION TO project_bs IS NULL ",
          prjct
        );
        ...
        this.subscription = this.router.events.subscribe((e) => {
          if (e instanceof NavigationEnd) {
            ...
            this.logger.log(
              "[AUTH-SERV] - NavigationEnd CURRENT-URL SEGMENTS ",
              url_segments
            );
            ...
            if (this.nav_project_id === undefined) {
              this.logger.log(
                "[AUTH-SERV] - CURRENT-URL SEGMENTS > NAVIGATION-PROJECT-ID IS UNDEFINED 1",
                this.nav_project_id,
            ...
            if (
              ...
              this.logger.log(
                "[AUTH-SERV] NAVIGATION-PROJECT-ID IS UNDEFINED 2",
                this.nav_project_id,
                " - UNSUBSCRIBE FROM ROUTER-EVENTS"
              );
              ...
              if (storedProjectJson) {
                ...
                this.logger.log(
                  "[AUTH-SERV] BEFORE TO PUBLISH this.project_bs.value ",
                  this.project_bs.value
                );
                if (this.project_bs.value == null) {
                  this.logger.log(
                    "[AUTH-SERV] PROJECT (get from storage) THAT IS PUBLISHED ",
                    project
                  );
                  this.project_bs.next(project);
                }
                ...
                this.logger.log(
                  "[AUTH-SERV] THERE IS NOT STORED PRJCT-JSON - FOR THE PROJECT WITH ID ",
                  this.nav_project_id,
                  "SEE AUTH GUARD"
                );
                ...
                this.logger.log(
                  "[AUTH-SERV] PROJECT THAT IS PUBLISHED (ONLY THE PROJECT ID BECAUSE THE PROJECT IS NOT PRESENT IN THE STORAGE)",
                  project
                );
  ...
  checkRoleForCurrentProjectPermissionOnlyToOwner() {
    ...
    this.logger.log(
      "[AUTH-SERV] - CHECK ROLE - JSON OF STORED PROJECT iD",
      project_id
    );
    this.logger.log(
      "[AUTH-SERV] - CHECK ROLE - JSON OF STORED PROJECT",
      storedProjectJson
    );
    if (storedProjectJson) {
      const storedProjectObject = JSON.parse(storedProjectJson);
  ...
  checkRoleForCurrentProjectAndRedirectAdminAndAgent() {
    this.logger.log(
      "[AUTH-SERV] - CHECK ROLE »»»»» CALLING CHECK-ROLE-FOR-CURRENT-PRJCT (USED X PRICING) AND BLOCK ADMIN AND AGENT"
    );
    ...
    return this._httpClient
      ...
      .then((res) => {
        ...
        if (jsonRes["success"] === true) {
          ...
          if (this.appConfigService.getConfig().firebaseAuth === true) {
            ...
            this.chat21CreateFirebaseCustomToken(jsonRes["token"]).subscribe(
              (fbtoken: string) => {
                ...
                this.logger.log(
                  "[AUTH-SERV] SSO - LOGIN 2. FIREBASE SIGNIN RESPO ",
                  fbtoken
                );
                ...
                if (fbtoken) {
                  ...
                  firebase
                    ...
                    .catch((error) => {
                      ...
                      this.logger.error(
                        "[AUTH-SERV] SSO - LOGIN - FIREBASE CUSTOM AUTH ERROR CODE ",
                        error
                      );
                    });
                } else {
            ...
            this.logger.log(
              "[AUTH-SERV] SSO - LOGIN - FIREBASE- AUTH false - !!!! SIGNIN WITHOUT FIREBASE CUSTOM TOKEN "
            );
          ...
          this.logger.error(
            "[AUTH-SERV] SSO - LOGIN - POST REQUEST ERROR jsonRes[success] NOT IS === true"
          );
      ...
      .catch((error) => {
        this.logger.error(
          "[AUTH-SERV] SSO - LOGIN - SIGNIN POST REQUEST ERROR",
          error
        );
        callback(error);
      });
  ...
  getPermission() {
    ...
    if (firebase.messaging.isSupported()) {
      ...
      Notification.requestPermission()
        .then((permission) => {
          this.logger.log(
            "[AUTH-SERV] SSO - LOGIN - 5B. >>>> getPermission Notification permission granted. - permission",
            permission
          );
          this.logger.log(
        ...
        .catch((err) => {
          this.logger.error(
            "[AUTH-SERV] SSO - LOGIN - 5C. >>>> getPermission Unable to get permission to notify.",
            err
          );
        });
    } else {
  ...
  updateToken(FCMcurrentToken) {
    ...
    this.logger.log(
      "[AUTH-SERV] Firebase Cloud Messaging  - Update token updates ",
      updates
    );
  ...
  emailVerify(user_id: string): Observable<User[]> {
    ...
    return this._httpClient.put<User[]>(url, JSON.stringify(body), httpOptions);
  }
  ...
  hasClickedGoToProjects() {
    ...
    this.logger.log(
      "[AUTH-SERV] - HAS CLICKED GO TO PROJECT - PRJCT VALUE = ",
      this.project_bs.value
    );
  ...
  showExpiredSessionPopup(showExpiredSessionPopup) {
    if (this.appConfigService.getConfig().firebaseAuth === true) {
      ...
      this.logger.log(
        "[AUTH-SERV]- SHOW EXPIRED SESSION POPUP - (USE CASE FIREBASE AUTH) ",
        this.show_ExpiredSessionPopup
      );
    } else {
      this.show_ExpiredSessionPopup = false;
      this.logger.log(
        "[AUTH-SERV] - SHOW EXPIRED SESSION POPUP - (USE CASE NO FIREBASE AUTH) ",
        this.show_ExpiredSessionPopup
      );
    }
  }
  ...
  userIsSignedIn(user_is_signed_in: boolean) {
    if (this.appConfigService.getConfig().firebaseAuth === true) {
      this.logger.log(
        "[AUTH-SERV] - USER-IS-SIGNED-IN - SHOW EXPIRED SESSION POPUP - (USE CASE FIREBASE AUTH) ",
        user_is_signed_in
      );
      ...
      this.logger.log(
        "[AUTH-SERV] - USER-IS-SIGNED-IN - SHOW EXPIRED SESSION POPUP - (USE CASE NO FIREBASE AUTH) - DOES NOT RUN this.notify.showExiperdSessionPopup"
      );
    }
  }
    ...
    if (this.appConfigService.getConfig().firebaseAuth === true) {
      ...
      this.notify.isOpenedExpiredSessionModal.subscribe(
        (isOpenedExpiredSession: boolean) => {
          this.logger.log(
            "[AUTH-SERV] - isOpenedExpiredSession ",
            isOpenedExpiredSession,
            "*** >>>> FCM is Supported: ",
            this.FCM_Supported
          );
          if (isOpenedExpiredSession) {
            this.logger.log(
              "[AUTH-SERV] - CHECK-IF-EXPIRED-SESSSION-MODAL-IS-OPENED (USE CASE FIREBASE) checkIfFCMIsSupported"
            );
      ...
      this.logger.log(
        "[AUTH-SERV]- CHECK-IF-EXPIRED-SESSSION-MODAL-IS-OPENED (USE CASE NO FIREBASE AUTH) DOES subscribe to isOpenedExpiredSessionModal"
      );
    }
  }
  ...
  hasOpenedLogoutModal(isOpenedlogoutModal: boolean) {
    this.logger.log(
      "[AUTH-SERV] - HAS-OPENED-LOGOUT-MODAL getConfig pushEngine",
      this.appConfigService.getConfig().pushEngine
    );
    if (this.appConfigService.getConfig().pushEngine === "firebase") {
      this.logger.log(
        "[AUTH-SERV] - HAS-OPENED-LOGOUT-MODAL - WORKS WITH FIREBASE  RUN checkIfFCMIsSupported"
      );
      this.logger.log(
        "[AUTH-SERV] - HAS-OPENED-LOGOUT-MODAL ",
        isOpenedlogoutModal,
        "*** >>>> FCM is Supported: ",
        this.FCM_Supported
      );
      ...
      this.logger.log(
        "[AUTH-SERV] - HAS-OPENED-LOGOUT-MODAL - WORKS WITHOUT FIREBASE DOES NOT RUN checkIfFCMIsSupported"
      );
    }
  }
  ...
  signOut(calledby: string) {
    ...
    if (calledby !== "account-settings" && calledby !== "autologin") {
      // in account-settings in tracked the acccont deleted event
      const storedUser = localStorage.getItem("user");
      ...
      if (currentUrlSegment[2] && currentUrlSegment[2].match(/^\d+/)[0]) {
        // regex exp to Check if String starts with Number -> .match(/^\d+/)[0]
          ...
          if (window["analytics"]) {
            try {
              window["analytics"].identify(storedUserParsed._id, {
                name:
                  storedUserParsed.firstname + " " + storedUserParsed.lastname,
            ...
            try {
              window["analytics"].track(
                ...
                {
                  username:
                    storedUserParsed.firstname +
                    " " +
                    storedUserParsed.lastname,
    ...
    // if (calledby !== 'autologin') {
    this.logger.log(
      "[AUTH-SERV] Signout this.router.url +++++ ",
      this.router.url
    );
    ...
    if (current_url.indexOf("request-for-panel") === -1) {
      this.logger.log(
        "[AUTH-SERV] Signout current url  NOT contains request-for-panel "
      );

      const chat_stored__currentUser = localStorage.getItem(
        chatPrefix + "__currentUser"
      );
      this.logger.log(
        "[AUTH-SERV] SIGNOUT - STORED chat_stored__currentUser : ",
        chat_stored__currentUser
      );
      if (chat_stored__currentUser) {
        localStorage.removeItem(chatPrefix + "__currentUser");
      }

      const chat_stored__tiledeskToken = localStorage.getItem(
        chatPrefix + "__tiledeskToken"
      );
      if (chat_stored__tiledeskToken) {
        localStorage.removeItem(chatPrefix + "__tiledeskToken");
        this.logger.log(
          "[AUTH-SERV] SIGNOUT - STORED chat_stored__tiledeskToken : ",
          chat_stored__currentUser
        );
      }
      ...
      this.logger.log(
        "[AUTH-SERV] Signout current url contains request-for-panel "
      );
    }
    ...
    if (this.appConfigService.getConfig().pushEngine === "firebase") {
      ...
      if (!this.APP_IS_DEV_MODE && this.FCM_Supported === true) {
        this.logger.log(
          "[AUTH-SERV] signOut this.FCMcurrentToken ",
        ...
        if (this.FCMcurrentToken !== undefined && this.userId !== undefined) {
          ...
          messaging
            ...
            .then((FCMtoken) => {
              ...
              this.logger.log(
                "[AUTH-SERV] signOut >>>> getToken storedUserObj ",
                storedUserObj
              );
              if (storedUserObj) {
  ...
  firebaseSignout(calledby) {
    ...
    firebase
      ...
      .then(
        ...
        function (error) {
          that.logger.error(
            "[AUTH-SERV] firebaseSignout SIGN-OUT - Error",
            error
          );
          // that.widgetReInit()
